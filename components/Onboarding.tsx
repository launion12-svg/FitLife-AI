...
export const Onboarding: React.FC<OnboardingProps> = ({ onComplete, error, initialProfile }) => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setProfile((prev) => ({
      ...prev,
      [name]: name === 'age' || name === 'weight' || name === 'height' || name === 'workoutDays' 
               ? parseInt(value, 10) 
               : value,
    }));
  };
            ...
            <div className="space-y-4">
             ...
             {profile.workoutLocation === 'Home' && (
              ...
              <label className="block pt-4">
                ...
                <input type="range" min="3" max="6" name="workoutDays" value={profile.workoutDays} onChange={handleChange} className="w-full h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer mt-2" />
                 <div className="text-center text-on-surface-secondary mt-1">{profile.workoutDays} {t('onboarding.days')}</div>
              </label>
            </div>
            <div className="flex justify-between mt-8">
              <button onClick={prevStep} className="bg-gray-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-gray-500">{t('onboarding.back')}</button>
              <button onClick={handleSubmit} className="bg-primary text-white font-bold py-2 px-4 rounded-lg hover:bg-primary-focus">{t('onboarding.generatePlan')}</button>
            </div>
          </div>
        );
      default:
