...
import type { WorkoutPlan, DailyWorkout, Exercise, ActiveWorkoutSession, SetLog } from '../src/types';
import { generateSpeech, getExerciseSubstitution } from '../src/services/geminiService';
...
const ActiveWorkoutScreen: React.FC<{
...
}> = ({ session, workout, onUpdateSession, onFinishWorkout, onUpdateExercise }) => {
    ...
    const isWorkoutComplete = workout.exercises.every(ex => {
        ...
        const numSets = parseInt(ex.sets.split('-')[0], 10);
    ...
    return (
        <div className="bg-surface p-4 rounded-lg shadow-lg">
            ...
            <p className="text-sm text-on-surface-secondary mb-4">{workout.duration}</p>
                ...
                return (
                    <div key={ex.id} className="border-t border-gray-700 py-4">
                        <div className="flex justify-between items-start">
                           ...
                           <button onClick={() => handleSubstitute(ex)} disabled={!!substitutingId} className="p-2 rounded-full bg-primary/20 text-primary hover:bg-primary/40 disabled:opacity-50 shrink-0">
                        ...
                        <div className="space-y-2 mt-3">
                            {Array.from({ length: numSets }).map((_, i) => (
                                <div key={i} className="flex items-center gap-2 text-sm">
                                    <span className="font-bold w-8">{t('workout.set')} {i+1}</span>
                                    <input type="number" placeholder="kg" onChange={e => handleInputChange(ex.id, 'weight', e.target.value)} defaultValue={exerciseLogs[i]?.weight || ''} disabled={exerciseLogs[i]?.completed} className="w-20 bg-background rounded p-1 text-center"/>
...
export const Workout: React.FC<WorkoutProps> = ({ plan, onUpdateWorkout }) => {
    const { t, language } = useLanguage();
    const { activeWorkout, startWorkout, updateActiveWorkout, finishWorkout } = useUserData();
                    ...
                    {plan.schedule.map((workoutDay, index) => (
                        <div key={index} className="bg-surface p-4 rounded-lg shadow-lg">
                            ...
                            <ul className="text-sm text-on-surface-secondary list-disc list-inside">
                                {workoutDay.exercises.map(e => <li key={e.name}>{e.name}</li>)}
                            </ul>
                        </div>
                    ))}
                </div>
